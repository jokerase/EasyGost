...
function Install_ct()
{
    ...
    `wget --no-check-certificate https://yunpan.sk222.cf/Cloud/gost.gz`
    `gunzip gost.gz`
    `mv gost-linux-"$bit"-"$ct_new_ver" gost`
    `mv gost /usr/bin/gost`
    `chmod -R 777 /usr/bin/gost`
    `wget --no-check-certificate https://raw.githubusercontent.com/jokerase/EasyGost/master/gost.service && chmod -R 777 gost.service && mv gost.service /usr/lib/systemd/system`
    `mkdir /etc/gost && wget --no-check-certificate https://raw.githubusercontent.com/jokerase/EasyGost/master/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost`
